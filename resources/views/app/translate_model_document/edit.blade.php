...
@section('sub_content')
    ...
    <div class="content">
        <div class="card bg-white">
            <div class="card-body">
                <form method="POST" action="{{ route('translate_model_document.update', $translateModelDocument->id) }}" enctype='multipart/form-data'>
                    @method('PATCH')
                    @csrf
                        ...
                        <div class="col-md-12">
                            ...
                            <select class="form-control {{ $errors->has('client_id') ? ' is-invalid' : '' }} client_id" id="client_id" name="client_id">
                                ...
                                @foreach ($assignTo as $assign)
                                    <option value="{{ $assign->id }}" {{ (old('client_id', $translateModelDocument->client_id) == $assign->id ? 'selected' : '') }}>{{ $assign->first_name . ' ' . $assign->last_name }}</option>
                                @endforeach
                            </select>
