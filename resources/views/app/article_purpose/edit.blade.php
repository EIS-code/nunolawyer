...
@section('sub_content')
    ...
    <div class="content">
        <div class="card bg-white">
            <div class="card-body">
                <form method="POST" action="{{ route('article_purpose.update', $articlePurpose->id) }}">
                    @method('PATCH')
                    @csrf
                    ...
                    <div class="form-group row">
                        <div class="col-md-12">
                            <label>{{ __('Text') }}</label>
                            <textarea id="text" type="text" class="form-control{{ $errors->has('text') ? ' is-invalid' : '' }}" name="text" required autofocus>{{ old('text', $articlePurpose->text) }}</textarea>
